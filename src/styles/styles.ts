// Navigation
export const navbar = "fixed top-0 left-0 w-full z-50 flex items-center justify-between px-10 py-5 border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-black/60 backdrop-blur-md"

export const navlink = "hover:text-zinc-500 transition font-medium";


// Sections
export const section = "py-20";

export const sectionCentered = `${section} text-center`;

export const container = "max-w-6xl px-6 mx-auto w-full";


// Text
export const title = "text-4xl sm:text-6xl font-bold mb-4 tracking-tight text-zinc-900 dark:text-white";

export const subtitle = "mt-4 text-2xl sm:text-2xl text-zinc-600 dark:text-zinc-300 max-w-2xl mx-auto";

export const sectionTitle = "text-4xl font-bold mb-4 text-zinc-900 dark:text-white";

export const paragraph = "mt-6 text-zinc-600 dark:text-zinc-400 leading-relaxed";

export const smallText = "text-sm mb-4 text-zinc-500 dark:text-zinc-400";

export const link = "hover:text-zinc-900 dark:hover:text-white transition";

export const footer = "py-10 text-center text-sm text-zinc-500 dark:text-zinc-400";


// Buttons
export const button = "px-6 py-3 rounded-xl transition";

export const buttonPrimary = `${button} bg-black text-white dark:bg-white dark:text-black font-medium hover:scale-105`;

export const buttonSecondary = `${button} border border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-white hover:bg-zinc-100 dark:hover:bg-zinc-900`;

export const buttonGroup = "mt-8 flex flex-col sm:flex-row gap-4 justify-center";


// Cards
export const grid = "grid grid-cols-2 md:grid-cols-3 gap-6";

export const card = "p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/50 hover:scale-[1.02] transition";

export const cardTitle = "font-semibold mb-2 text-zinc-900 dark:text-white";

export const cardText = "text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed";

// Lists
export const list = "space-y-2 text-zinc-600 dark:text-zinc-400 mx-auto text-left";

export const itemList = "flex items-center justify-between py-2 border-b border-zinc-200 dark:border-zinc-800";